#!/usr/bin/env python3
"""gen_ai_guide.py <folder> — write CLAUDE.md + .cursorrules tailored to the folder."""
import glob, os, sys

ROOT = os.path.abspath(sys.argv[1]) if len(sys.argv) > 1 else "."
pages = sorted(os.path.relpath(p, ROOT) for p in glob.glob(f"{ROOT}/**/*.html", recursive=True))
css = "css/styles.css" if os.path.exists(f"{ROOT}/css/styles.css") else "the main CSS file"

GUIDE = f"""# AI assistant guide
...
## Where things live
- `{css}` — the design: tokens (`:root`) + all component styles. Change colors/fonts/
...
- Pages ({len(pages)}): {", ".join(pages[:14])}{" …" if len(pages) > 14 else ""}
...
## Customize design via tokens (in `{css}` `:root`)
```css
...
```
"""
open(f"{ROOT}/CLAUDE.md", "w", encoding="utf-8").write(GUIDE)
open(f"{ROOT}/.cursorrules", "w", encoding="utf-8").write(GUIDE)
print("  wrote CLAUDE.md + .cursorrules")
